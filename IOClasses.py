...
if __name__ == '__main__':
    raise Exception('This f is not meant to run by itself')

import DataClasses as dc
import ProcessingClasses as pc

class FileIO:
    """Processes data to and from f:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        Args:
            file_name (list): list of f names [CD Inventory, Track Inventory] that hold the data.
        ...
        try:
            with open(filename_CD, 'w') as f:
                for disc in lst_Inventory:
                    f.write(disc.get_record())

            with open (filename_Track, 'w') as f:
                for disc in lst_Inventory:
                    cd_id = disc.cd_id
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            with open(filename_CD, 'r') as f:
                for line in f:
                    data = line.strip().split(',')
                    row = dc.CD(data[0], data[1], data[2])
                    lst_Inventory.append(row)

            with open(filename_Track, 'r') as f:
                for line in f:
                    data = line.strip().split(',')
